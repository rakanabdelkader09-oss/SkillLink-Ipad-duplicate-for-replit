...
// This is the CRITICAL line for sideloaded IPAs. 
// It tells the build tool to include the video file in the app bundle.
...
export function PaperCraftsScreen({ onBack }: PaperCraftsScreenProps) {
  ...
  // Aggressive Play Hook for iOS 16 WKWebView
  useEffect(() => {
    if (videoRef.current) {
      ...
      if (playPromise !== undefined) {
        playPromise.catch(() => {
          // If auto-play is blocked, we force it on the first touch event
          const forcePlay = () => {
            video.play();
  ...
  return (
    <div className="h-full bg-background overflow-y-auto pb-6">
      ...
      <div className="relative h-56 bg-gradient-to-br from-amber-200 to-orange-300 overflow-hidden">
        ...
        >
          {/* Using the imported 'headerVideo' variable handles the internal file path automatically */}
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent flex flex-col items-center justify-end pb-6 px-6 pointer-events-none z-10">
          <div className="text-center">
